...
class TrajectoryTracker:
    ...
    def compute_control(self, x, y, th, t):
        ...
        u2 = ydd_d + self.kpy*(y_d - y) + self.kdx*(yd_d - y_dot)
        eqn1 = np.array([np.cos(th), - self.V_prev*np.sin(th)])
        eqn2 = np.array([np.sin(th), self.V_prev*np.cos(th)])
        eqns = np.array([eqn1, eqn2])
        sol = np.array([u1, u2])
        ans = np.linalg.solve(eqns, sol)
        om = ans[1]
        a = ans[0]
        V = self.V_prev + a*dt
        if V < V_PREV_THRES:
