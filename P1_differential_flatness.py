...
def compute_traj_coeffs(initial_state, final_state, tf):
    ...
    eqn1 = np.array([1, 0, 0, 0, 0, 0, 0, 0])
    eqn2 = np.array([1, tf, tf**2, tf**3, 0, 0, 0, 0])
    eqn3 = np.array([0, 1, 0, 0, 0, 0, 0, 0])
    eqn4 = np.array([0, 1, 2*tf, 3*tf**2, 0, 0, 0, 0])
    eqn5 = np.array([0, 0, 0, 0, 1, 0, 0, 0])
    eqn6 = np.array([0, 0, 0, 0, 1, tf, tf**2, tf**3])
    eqn7 = np.array([0, 0, 0, 0, 0, 1, 0, 0])
    eqn8 = np.array([0, 0, 0, 0, 0, 1, 2*tf, 3*tf**2])
    eqn_mat = np.array([eqn1, eqn2, eqn3, eqn4, eqn5, eqn6, eqn7, eqn8])
    soln = np.array([initial_state.x, final_state.x, initial_state.xd, final_state.xd, initial_state.y, final_state.y, initial_state.yd, final_state.yd])
    coeffs = np.linalg.solve(eqn_mat, soln)
    ########## Code ends here ##########
    return coeffs
...
def compute_traj(coeffs, tf, N):
    ...
    ########## Code starts here ##########
    '''
    traj index definition, x=0, y=1, th=2, xdot=3, ydot=4, xddot=5, yddot=6
    '''
    for i in range(N):
            traj[i,0] = np.array([coeffs[0] + coeffs[1]*t[i] + coeffs[2]*t[i]**2 + coeffs[3]*t[i]**3])
            traj[i,1] = np.array([coeffs[4] + coeffs[5]*t[i] + coeffs[6]*t[i]**2 + coeffs[7]*t[i]**3])
            traj[i,3] = np.array([coeffs[1] + 2*coeffs[2]*t[i] + 3*coeffs[3]*t[i]**2])
            traj[i,4] = np.array([coeffs[5] + 2*coeffs[6]*t[i] + 3*coeffs[7]*t[i]**2])
            traj[i,2] = np.array([np.arctan2(traj[i, 4], traj[i, 3])])
            traj[i,5] = np.array([2*coeffs[2] + 6*coeffs[3]*t[i]])
            traj[i,6] = np.array([2*coeffs[6] + 6*coeffs[7]*t[i]])
    ########## Code ends here ##########
...
def compute_controls(traj):
    ...
    om = np.zeros(N)
    for i in range(N):
        if np.sin(traj[i, 2]) == 0:
            V[i] = traj[i, 3]/np.cos(traj[i, 2])
        else:
            V[i] = traj[i, 4]/np.sin(traj[i, 2])
        eqn1 = np.array([np.cos(traj[i, 2]), -V[i]*np.sin(traj[i, 2])])
        eqn2 = np.array([np.sin(traj[i, 2]), V[i]*np.cos(traj[i, 2])])
        sol = np.array([traj[i, 5], traj[i, 6]])
        eqn_mat = np.array([eqn1, eqn2])
        ans = np.linalg.solve(eqn_mat, sol)
        om[i] = ans[1]
    ########## Code ends here ##########
...
def rescale_V(V, om, V_max, om_max):
    ...
    """
    ########## Code starts here ##########
    V_tilde = np.zeros(np.shape(V)[0])
    for i in range(np.shape(V)[0]):
        V_tilde[i] = min(abs(V[i]), V_max, abs(om_max*V[i]/om[i]))
    ########## Code ends here ##########
    return V_tilde
